...
// Dropdown: products for orderdetails forms
app.get('/api/products/dropdown', (req, res) => {
    const query = 'SELECT productID, productName FROM Products ORDER BY productName;';
    db.pool.query(query, (err, results) => {
        if (err) return res.status(500).json({ error: err.message });
        res.json(results);
    });
...
app.delete('/api/orderdetails/:id', (req, res) => {
    ...
    db.pool.query('CALL sp_delete_order_details(?)', [txnID], (err, result) => {
        if (err) {
            console.error(err);
            return res.status(500).json({ error: "Stored procedure runtime failure." });
        }
        ...
        return res.json({ message: `Order detail record ${txnID} successfully deleted.` });
    });
});
...
// Read Products table, joins vendorname
